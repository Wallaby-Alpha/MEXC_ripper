...
def test_weex_symbol_resolver_mapping():
    ...
    assert resolver.resolve("SUIUSDT") == "cmt_suiusdt"
    assert resolver.resolve("suiusdt") == "cmt_suiusdt"
    assert resolver.resolve("BTCUSDT") == "cmt_btcusdt"
    assert resolver.resolve("ENAUSDT") == "cmt_enausdt"

    # 2. Multiplier coin mapping (PEPE -> 1000PEPE)
    assert resolver.resolve("PEPEUSDT") == "cmt_1000pepeusdt"
    assert resolver.resolve("1000PEPEUSDT") == "cmt_1000pepeusdt"
    ...
    formatted_btc_p = resolver.format_price("cmt_btcusdt", 62450.384)
    assert formatted_btc_p == "62450.4"

    formatted_sui_p = resolver.format_price("cmt_suiusdt", 1.854382)
    assert formatted_sui_p == "1.8544"

    # 5. Size increment formatting
    formatted_sui_sz = resolver.format_size("cmt_suiusdt", 543.82)
...
def test_weex_executor_unlisted_coin_safety():
    ...
    resolver = WeexSymbolResolver(auto_fetch=False)
    resolver.contract_map = {"cmt_suiusdt": {"symbol": "cmt_suiusdt", "tick_size": 4, "size_increment": 1.0, "min_order_size": 1.0}}
    resolver.alias_to_canonical = {"suiusdt": "cmt_suiusdt"}
